...
//					System.out.println("open");
		...
		System.out.println("It took " + (endTime - startTime) + " milliseconds" + " Finally Loaded" );
...
//		HashMap<String, String> map = Util.getOutagesMap();
		HashMap<String, String> map = Util.getResOutagesMap(); //set to load the new TS outages
...
//			List<CwmsTableData> data = Util.getDbOutages(end, start, loc, "Unk");
			List<CwmsTableData> data = Util.getDbOutages(end, start, loc, "n/a");
			allOutages.add(data);
		}
...
//					int x = td.getUnit();//old way
					ArrayList x = td.getUnits();
					// System.out.println(date + " " + id + " " + num + " " + d
					// + " " + ts + " " + x);
//					loadoutagesMap(id, d, ts, x);
					loadNewOutagesMap(id, d, ts, x);
				}
			}
		}
//		setOutagesMap(outMap);
	 ...
	 * @param units
	 */
	//TODO changes for new time series per project
	private void loadNewOutagesMap(String locId, String date, String ts, ArrayList units) {
		// date, locationID, vector of outages
		LinkedHashMap<String, LinkedHashMap<String, Vector<OutagesTable>>> outMap = getOutagesMap();
		OutagesTable outageTable = new OutagesTable(locId, date, ts, units.toArray());
		...
		if (outMap.containsKey(date)) { // already has the date
			...
			if (outages.containsKey(locId)) {// Check for locationId, exists, add outage
...
//		HashMap<String, String> spcMap = sfd.getSpcMap();// 160417=0.00,
		...
		table.setValueAt("", SITES_WATER_SCHEDULE, count + 1); //Removed SPC
				...
				if (dly.containsKey(key)) {
					...
					if (data.getRescheduleOpNum().doubleValue() > 0.0D) {
						...
						if (data.getRescheduleOpNum().doubleValue() > data.getODNum().doubleValue()) {
							...
							//Reschedule when you put on the powerhouse (OP) line to get a reschedule to show up.
...
//					}
	...
	private void loadPoolElevationInfo(JTable table, Calendar startDay, Calendar endDay, int TOTAL_COL,
		...
		// TODO outages changes needed
		LinkedHashMap<String, LinkedHashMap<String, Vector<OutagesTable>>> oMap = getOutagesMap();
//		LinkedHashMap<String, LinkedHashMap<String, Vector<OutagesTable>>> oMap = getResOutagesMap();
	 ...
	 * @return
	 */
	protected static String[] formatOutagesDayMap(LinkedHashMap<String,
												  LinkedHashMap<String, 
												  Vector<OutagesTable>>> tbl,
												  String outageKey) {
		String[] sites = { "", "", "", "", "", "" };
...
//				String unit = site.getUnit();
				Object[] units = site.getUnits(); //changed for new timeseries
				Object data = null;
				StringBuffer values = new StringBuffer();;
				for(int y = 0; y<units.length; y++){
					data = units[y];
					values.append(String.valueOf(units[y]));
				}
//				System.out.println(loc + ": " + String.valueOf(data));
				System.out.println(loc + ": " + String.valueOf(values));
				
				if (loc.equals("FTPK")) {
//					sites[0] = sites[0].concat(units.toString());
					sites[0] = sites[0].concat(String.valueOf(values));
				} else if (loc.equals("GARR")) {
//					sites[1] = sites[1].concat(units.toString());
					sites[1] = sites[1].concat(String.valueOf(values));
				} else if (loc.equals("OAHE")) {
//					sites[2] = sites[2].concat(units.toString());
					sites[2] = sites[2].concat(String.valueOf(values));
				} else if (loc.equals("BEND")) {
//					sites[3] = sites[3].concat(units.toString());
					sites[3] = sites[3].concat(String.valueOf(values));
				} else if (loc.equals("FTRA")) {
//					sites[4] = sites[4].concat(units.toString());
					sites[4] = sites[4].concat(String.valueOf(values));
				} else if (loc.equals("GAPT")) {
//					sites[5] = sites[5].concat(units.toString());
					sites[5] = sites[5].concat(String.valueOf(values));
				}
			}
		}
		return sites;
